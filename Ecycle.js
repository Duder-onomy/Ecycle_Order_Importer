...
var uploadFolder = DocsList.getFolderById("0B8yxm0Ut_NZINk85Wjg4VTFmOXM");
...
uploadSheet.getRange("K:K").copyTo(uploadSheet.getRange("A:A"));
//Name 2
uploadSheet.insertColumnAfter(1);
uploadSheet.getRange("M:M").copyTo(uploadSheet.getRange("B:B"));
//Addy 1
uploadSheet.insertColumnAfter(2);
uploadSheet.getRange("O:O").copyTo(uploadSheet.getRange("C:C"));
//Addy 2
uploadSheet.insertColumnAfter(3);
uploadSheet.getRange("Q:Q").copyTo(uploadSheet.getRange("D:D"));
//Addy 3
uploadSheet.insertColumnAfter(4);
//town / city
uploadSheet.insertColumnAfter(5);
uploadSheet.getRange("T:T").copyTo(uploadSheet.getRange("F:F"));
//country
uploadSheet.insertColumnAfter(6);
uploadSheet.getRange("X:X").copyTo(uploadSheet.getRange("G:G"));
//Zip
uploadSheet.insertColumnAfter(7);
uploadSheet.getRange("X:X").copyTo(uploadSheet.getRange("H:H"));
for (var i = 0 ; i < 50 ; i++)
  {
    var cell = uploadSheet.getRange(i + 2 , 8);
    var country = uploadSheet.getRange(i + 2 , 7).getValue();
    if (country == "US")
    {
      var cellValue = cell.getValue();
      var newValue = cellValue.substring(0,5);
      cell.setValue(Utilities.formatString('%05d',newValue.toString()));
    }
  }
//Telephone
